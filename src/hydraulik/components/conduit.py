...
Genau EINE Hydraulik-Angabe (oder keine):
  ...
  - length (+ d_inner, …)   → Rohrmodell (Churchill), optional Wärmeverlust
"""
...
class Conduit(TwoPortComponent):
    ...
    def check_params(self):
        ...
        modes = sum([has_c, has_dp and has_q, has_len])
        if modes > 1:
            errs.append("Höchstens EINE Hydraulik-Angabe: 'c_Pa_m3h2' ODER "
                        "Auslegungspunkt 'dp_kPa'+'q_m3h' ODER 'length_m' (Rohrmodell).")
    ...
    def thermal_outlet(self, t_in: float, m_dot: float, fluid: Fluid) -> ThermalResult:
        if self.length is None or self.u_linear <= 0.0 or m_dot <= 0.0:
